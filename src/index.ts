...
import { showTerm, LitRecord, abs, PVar, Var, Term, PAnn, Lit, app, RecordSelect } from './terms';
...
//const s = '\\x y -> x';
const term = RecordSelect('x', LitRecord(fromArray([['x', abs([PAnn(PVar('y'), tforall([['t', null]], tfun(TVar('t'), TVar('t'))))], Var('y'))]]))); //parseTerm(s);
